...
- `milos-armor-hud` (dir: `ukus-armor-hud-0.10.2+mc1.21.11`) — HUD widget showing equipped armor + low-durability warning. Forked from `armor_hud`/`ukus-armor-hud`.
...
The directory name for the armor HUD mod doesn't match its mod id (`milos-armor-hud`) or package (`ru.berdinskiybear.armorhud`) — it's a holdover from before the Milo fork/rename; don't assume directory name == mod id for that one.
...
- Run a single test class: `./gradlew test --tests "ru.berdinskiybear.michud.icon.MicStateTest"`
...
- **Package root**: `ru.berdinskiybear.<shortname>`, matching `maven_group` in `gradle.properties`.
