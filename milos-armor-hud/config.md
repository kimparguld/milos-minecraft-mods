## Configuration
Here you can find description of all parameters for **Milo's Armor HUD** mod. Configuration file
...
> **Note:** if you're upgrading from an older version of this mod (`uku's Armor HUD` / `ukus-armor-hud`), your saved settings will reset to defaults — the mod id changed as part of the rebrand to Milo's Armor HUD, which changed the config file name. Your old config file is safe to delete.
...
1. ##### `"warningShown"`
    ...
    * Default value: `true`
1. ##### `"playBreakSound"`
    * If enabled, a sound plays when a displayed item's durability drops to the warning threshold
      (same threshold as `minDurabilityValue` / `minDurabilityPercentage` below).
    * Default value: `true`
...
1. ##### `"minDurabilityPercentage"`
    ...
    * Note: this same threshold also determines which pieces are shown when `"widgetShown"` is set to `"damaged_pieces"`.
    * Default value: `0.3`
